...
if (preg_match('/[A-Za-z0-9]+ad/', $_POST['username']) == 0) {
    die ('<h2>Please add valid username must have a "ad" at end</h2>');
}
...
if ($stmt = $con->prepare('INSERT INTO admin (username, password, phone, name) VALUES (?, ?, ?, ?)')) {
	// We do not want to expose passwords in our database, so hash the password and use password_verify when a user logs in.
	$password = password_hash($_POST['password'], PASSWORD_DEFAULT);
	$stmt->bind_param('ssss', $_POST['username'], $password, $_POST['phone'], $_POST['name']);
	$stmt->execute();
	echo '<script>alert("Administrator added. Now HE or SHE is an admin member")</script>';
